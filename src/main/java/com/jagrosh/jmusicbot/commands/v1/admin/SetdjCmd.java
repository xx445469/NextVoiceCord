...
public class SetdjCmd extends AdminCommand
{
    ...
    protected void execute(CommandEvent event)
    {
        ...
        {
            event.reply(event.getClient().getError()+" " + bot.msg(event.getGuild(), "settings.dj.missingArg"));
            return;
        }
        ...
        {
            s.setDJRole(null);
            event.reply(event.getClient().getSuccess()+" " + bot.msg(event.getGuild(), "settings.dj.cleared"));
        }
        else
        {
            ...
            else
            {
                ...
                event.reply(event.getClient().getSuccess()+" " + bot.msg(event.getGuild(), "settings.dj.set", list.get(0).getName()));
            }
        }
    }
